...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location is not hive:
            if location.bees and counter >= self.min_range and counter <= self.max_range:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            while location:
                if location.ant and location.ant.is_container:
                    if location.ant.contained_ant and location.ant.contained_ant not in self.buffed_ants:
                        location.ant.contained_ant.damage *=2
                        self.buffed_ants.append(location.ant.contained_ant)
                if location.ant and location.ant not in self.buffed_ants:
                    location.ant.damage *= 2
                    self.buffed_ants.append(location.ant)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and not target.been_spooked:
            apply_effect(make_scare, target, 2)
            target.been_spooked = True
        # END Problem EC
